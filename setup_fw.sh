...
function install_firewall_service(){
    ...
    cat > $dir_systemd/firewall.service <<EOF
[Unit]
...
Type=oneshot
RemainAfterExit=yes
ExecStart=$dir_fw_rules/fw_on.sh start
ExecStop=$dir_fw_rules/fw_off.sh stop

[Install]
WantedBy=multi-user.target
EOF
chmod +x $dir_fw_rules/fw_on.sh
chmod +x $dir_fw_rules/fw_off.sh
...
if systemctl list-units --full -all | grep -Fq "apache2" || systemctl list-units --full -all | grep -Fq "nginx"; then
...
cat > $dir_fw_rules/fw_off.sh <<EOF
#!/bin/bash
